...
frontend/
  ...
  components/
    ...
    Auth.js         — /auth: табы «Войти»/«Зарегистрироваться» + форма «Забыли пароль» + экран «Проверьте почту».
                      ...
                      Регистрация требует чекбокс согласия на обработку перс. данных; ссылка открывает модалку с текстом соглашения (openConsentModal).
    ...
    Profile.js      — /profile: hero (аватар-инициалы + приветствие + «Выйти» на зелёном градиенте);
                      ...
                      У каждого заказа кнопка «Повторить» (↻) — POST /orders теми же items/total, затем перезагрузка списка (loadOrders).
    ...
    Products.js / ProductDetail.js — товары (3 шт.); счётчик [− N кг +] с прямым вводом, шаг 10 кг, макс 1000 кг (опт)
